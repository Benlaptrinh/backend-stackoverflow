...
const mongoose = require('mongoose');
const connectDB = require('./config/db');
const { redis, initRedis } = require('./utils/redis'); // ✅ chỉ 1 lần
...
if (typeof generalLimiter === 'function') app.use(generalLimiter);
...
(async () => {
    await connectDB();
    await initRedis();                              // ✅ connect redis trước khi listen
    server.listen(PORT, () => console.log(`API listening on :${PORT}`));
})();
